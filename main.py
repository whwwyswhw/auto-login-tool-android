...
class PK10App:
    def __init__(self, page: ft.Page):
        ...
        self._load_balls()
        self._build_ui()
        self._log("Flet 版启动完成", "SUCCESS")
    
    def _load_balls(self):
        for num, b64_str in _EMBEDDED_BALLS.items():
            try:
                self.ball_images[num] = ft.Image(
                    src_base64=b64_str, width=30, height=30, fit=ft.ImageFit.CONTAIN
                )
    ...
    def _build_ui(self):
        # 标题
        self.page.add(
            ft.Text("PK10 投注助手", size=24, weight=ft.FontWeight.BOLD, color="#E6EDF3"),
            ft.Text("完整版（含 10 张号码球图片）", size=14, color="#8B949E"),
            ft.Divider(),
        )
        
        # 号码球展示
        ball_row = ft.Row(spacing=4, wrap=True)
        for n in range(1, 11):
            ns = str(n)
        ...
        # 功能按钮
        self.page.add(
            ft.ElevatedButton("测试登录", on_click=lambda e: self._log("登录功能开发中...", "INFO"), height=36),
            ft.ElevatedButton("刷新开奖", on_click=lambda e: self._log("刷新功能开发中...", "INFO"), height=36),
        )
        
        # 日志区
        self.log_view = ft.ListView(height=200, spacing=2, auto_scroll=True)
        self.page.add(ft.Divider(), ft.Text("操作日志", size=14, weight=ft.FontWeight.BOLD, color="#E6EDF3"), self.log_view)
